...
public class ADSR {
	...
	public double envGenNew(int envIndex,double attack, double decay, double sustainTime, double sustainLevel, double release,double SR) {
	   ...
	   envValueCurr=getEnvValue();
		...
		envValueCurr=envValueCurr+1.0/(double)attackSamples;  //rising slope
		}
		else if ((envIndex>=attackSamples) && (envIndex<decaySamples)) { 
		envValueCurr=zeta*envValueCurr+(1-zeta)*sustainLevel;   //decay slope
		}
		else if ((envIndex>=decaySamples) && (envIndex<sustainSamples)) { 
		envValueCurr=sustainLevel;  //sustain
			}
		else if ((envIndex>=sustainSamples) && (envIndex<totalTime)) { 
		envValueCurr=zetaR*envValueCurr;   //release slope
